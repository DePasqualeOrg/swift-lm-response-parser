...
`LMResponseParserMLX` offers two layers of integration:

- ``ResponseChatSession``: multi-turn session with built-in tool dispatch, mirrors `MLXLMCommon.ChatSession` but yields ``/LMResponseParser/ResponseOutputItem`` snapshots or ``/LMResponseParser/ResponseStreamingEvent`` values instead of plain strings.
...
One session call corresponds to one assistant turn and produces one ``/LMResponseParser/Response``, regardless of how many internal generation passes happen for tool dispatch.
...
``ResponseChatSession`` owns the restart loop for tool use. When the model emits a completed tool call and `toolDispatch` is set, the session runs the callback, appends the returned ``/LMResponseParser/ResponseFunctionCallOutput/Output`` as a function-call output, and starts the next internal generation pass.
...
Use ``streamResponseEvents(input:cache:parameters:context:modelType:modelConfig:format:config:priorOutput:wiredMemoryTicket:)`` instead when you want each ``/LMResponseParser/ResponseStreamingEvent`` directly – useful for SSE serialization or proxying.
