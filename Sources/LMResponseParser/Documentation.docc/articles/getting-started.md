...
`ParserTokenizer` declares three methods (`convertTokenToId`, `encode`, `decode`) whose signatures match `MLXLMCommon.Tokenizer` exactly. Any conforming concrete type already satisfies `ParserTokenizer`'s requirements; conform with a single empty extension:
...
- [MLX integration](/documentation/lmresponseparsermlx): high-level chat sessions on top of MLX Swift LM.
