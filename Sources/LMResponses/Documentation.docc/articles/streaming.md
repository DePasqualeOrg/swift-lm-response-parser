...
``ResponseStream`` is the engine-agnostic streaming entry point. Configure it with a ``ResponseFormat``, a ``ResponseTokenizer``, and the tools the model has been told about, then feed it tokens one at a time. After each token it exposes both the latest accumulated items and the streaming events the token produced.

For MLX, the higher-level conveniences in the [`LMResponsesMLX`](/documentation/lmresponsesmlx) module wrap this loop so callers don't manage tokens themselves.
