...
struct ChatTemplateRoundTripTests {
  ...
  private func mistralParserCallId() throws -> String {
    let items = parseResponse(
      ...
      format: .mistral,
      tokenizer: NullResponseTokenizer(),
    )
    for item in items {
...
/// Tokenizer satisfying `ResponseTokenizer`'s minimal surface for parsers
...
private struct NullResponseTokenizer: ResponseTokenizer {
